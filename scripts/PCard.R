...
pcard16<-pcard15 %>% 
  mutate(MScore=ifelse(`% Non-Genetic Removals`<=15,3,
         ...
         PScore=ifelse(is.na(`% Missing Scores`),3,
                       ifelse(`% Missing Scores`<=2,3,
         ...
         CScore=ifelse(`Boar Stud`%in%c('SPGNC'),3, 
                       ifelse(as.integer(`Trash Rate`)%in%c(5:15),3,
         ...
         LScore=ifelse(`Boar Stud`%in%c('Skyline Boar Stud','Norson','SPGVA','SPGNC'),3,LScoreb),
